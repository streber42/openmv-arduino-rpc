...
uint16_t rpc::__crc_16(uint8_t *data, size_t size) {
  ...
  //    crc ^= data[i] << 8;
  //    for (size_t j = 0; j < 8; j++) crc = (crc << 1) ^ ((crc & 0x8000) ?
  //    0x1021 : 0x0000);
  // }

  for (size_t i = 0; i < size; i++)
    crc = pgm_read_word_near(__crc_16_table + (((crc >> 8) ^ data[i]) & 0xff)) ^
          (crc << 8);

  return crc;
}

void rpc::_zero(uint8_t *data, size_t size) { memset(data, 0, size); }
...
bool rpc::_get_packet(uint16_t magic_value, uint8_t *buff, size_t size,
...
void rpc::_set_packet(uint8_t *buff, uint16_t magic_value, uint8_t *data,
                      size_t size) {
  buff[0] = magic_value;
...
bool rpc::stream_reader_loop(rpc_stream_reader_callback_t callback,
...
bool rpc::stream_writer_setup() {
  ...
  if ((magic != 0xEDF6) && (crc != __crc_16(packet, sizeof(packet) - 2)))
    return false;
  __queue_depth = max(
      min(unpack_unsigned_long(packet + 2), _stream_writer_queue_depth_max()),
      1);
  __rx_lfsr = 255;
...
bool rpc::stream_writer_loop(rpc_stream_writer_callback_t callback,
...
bool rpc_master::__put_command(uint32_t command, uint8_t *data, size_t size,
                               unsigned long timeout) {
  Serial.print("1");
  ...
  while ((millis() - start) < timeout) {
    Serial.print("3");
    _set_packet(out_header, _COMMAND_HEADER_PACKET_MAGIC, (uint8_t *)header,
                8);  // set repeatedly because SPI.transfer() clears.
    _set_packet(__buff, _COMMAND_DATA_PACKET_MAGIC, data,
                size);  // set repeatedly because SPI.transfer() clears.
    ...
    if (_get_packet(_COMMAND_HEADER_PACKET_MAGIC, __in_command_header_buf,
                    sizeof(__in_command_header_buf), _get_short_timeout)) {
      put_bytes(__buff, size + 4, _put_long_timeout);
      if (_get_packet(_COMMAND_DATA_PACKET_MAGIC, __in_command_data_buf,
                      sizeof(__in_command_data_buf), _get_short_timeout)) {
        Serial.println("4");
        return true;
      }
    }
...
bool rpc_master::__get_result(uint8_t **data, size_t *size,
                              unsigned long timeout) {
                                Serial.print("6");
  _put_short_timeout = _put_short_timeout_reset;
  _get_short_timeout = _get_short_timeout_reset;
  ...
  while ((millis() - start) < timeout) {
    Serial.print("7");
    _set_packet(__out_result_header_ack, _RESULT_HEADER_PACKET_MAGIC, NULL,
                0);  // set repeatedly because SPI.transfer() clears.
    _set_packet(__out_result_data_ack, _RESULT_DATA_PACKET_MAGIC, NULL,
                0);  // set repeatedly because SPI.transfer() clears.
    _zero(__in_result_header_buf, sizeof(__in_result_header_buf));
    _flush();
    put_bytes(__out_result_header_ack, sizeof(__out_result_header_ack),
              _put_short_timeout);
    if (_get_packet(_RESULT_HEADER_PACKET_MAGIC, __in_result_header_buf,
                    sizeof(__in_result_header_buf), _get_short_timeout)) {
      uint32_t in_result_data_buf_len =
          unpack_unsigned_long(__in_result_header_buf + 2) + 4;
      if (__buff_len < in_result_data_buf_len) {
        Serial.println("8");
        return false;
      }
      put_bytes(__out_result_data_ack, sizeof(__out_result_data_ack),
                _put_short_timeout);
      if (_get_packet(_RESULT_DATA_PACKET_MAGIC, __buff, in_result_data_buf_len,
                      _get_long_timeout)) {
        *data = __buff + 2;
...
bool rpc_master::call_no_copy_no_args(const __FlashStringHelper *name,
  ...
  return __put_command(_hash(name), NULL, 0, send_timeout)
             ? __get_result((uint8_t **)result_data, result_data_len,
                            recv_timeout)
             : false;
}

bool rpc_master::call_no_copy_no_args(const String &name, void **result_data,
                                      size_t *result_data_len,
                                      unsigned long send_timeout,
                                      unsigned long recv_timeout) {
  return __put_command(_hash(name.c_str(), name.length()), NULL, 0,
                       send_timeout)
             ? __get_result((uint8_t **)result_data, result_data_len,
                            recv_timeout)
             : false;
...
bool rpc_master::call_no_copy_no_args(const char *name, void **result_data,
                                      size_t *result_data_len,
                                      unsigned long send_timeout,
                                      unsigned long recv_timeout) {
  return __put_command(_hash(name), NULL, 0, send_timeout)
...
bool rpc_master::call_no_copy(const __FlashStringHelper *name,
                              ...
                              unsigned long send_timeout,
                              unsigned long recv_timeout) {
  return __put_command(_hash(name), (uint8_t *)command_data, command_data_len,
                       send_timeout)
             ? __get_result((uint8_t **)result_data, result_data_len,
                            recv_timeout)
             : false;
...
bool rpc_master::call_no_copy(const String &name, void *command_data,
                              size_t command_data_len, void **result_data,
                              size_t *result_data_len,
                              unsigned long send_timeout,
                              unsigned long recv_timeout) {
  return __put_command(_hash(name.c_str(), name.length()),
                       (uint8_t *)command_data, command_data_len, send_timeout)
             ? __get_result((uint8_t **)result_data, result_data_len,
                            recv_timeout)
             : false;
}

bool rpc_master::call_no_copy(const char *name, void *command_data,
                              size_t command_data_len, void **result_data,
                              size_t *result_data_len,
                              unsigned long send_timeout,
                              unsigned long recv_timeout) {
...
bool rpc_master::call_no_args(const __FlashStringHelper *name,
                              void *result_data, size_t result_data_len,
                              bool return_false_if_received_data_is_zero,
                              unsigned long send_timeout,
                              unsigned long recv_timeout) {
  void *result_pointer;
  size_t result_size;
  bool result = __put_command(_hash(name), NULL, 0, send_timeout)
                    ? __get_result((uint8_t **)&result_pointer, &result_size,
                                   recv_timeout)
                    : false;
...
bool rpc_master::call_no_args(const String &name, void *result_data,
                              size_t result_data_len,
                              bool return_false_if_received_data_is_zero,
                              unsigned long send_timeout,
                              unsigned long recv_timeout) {
  ...
  size_t result_size;
  bool result =
      __put_command(_hash(name.c_str(), name.length()), NULL, 0, send_timeout)
          ? __get_result((uint8_t **)&result_pointer, &result_size,
                         recv_timeout)
          : false;
  if (return_false_if_received_data_is_zero) result = result && result_size;
...
bool rpc_master::call_no_args(const char *name, void *result_data,
                              size_t result_data_len,
                              bool return_false_if_received_data_is_zero,
                              unsigned long send_timeout,
                              unsigned long recv_timeout) {
...
bool rpc_master::call(const __FlashStringHelper *name, void *command_data,
                      size_t command_data_len, void *result_data,
                      size_t result_data_len,
                      bool return_false_if_received_data_is_zero,
  ...
  void *result_pointer;
  size_t result_size;
  bool result = __put_command(_hash(name), (uint8_t *)command_data,
                              command_data_len, send_timeout)
                    ? __get_result((uint8_t **)&result_pointer, &result_size,
                                   recv_timeout)
                    : false;
  if (return_false_if_received_data_is_zero) result = result && result_size;
...
bool rpc_master::call(const String &name, void *command_data,
                      size_t command_data_len, void *result_data,
                      size_t result_data_len,
                      bool return_false_if_received_data_is_zero,
                      unsigned long send_timeout, unsigned long recv_timeout) {
  ...
  size_t result_size;
  bool result =
      __put_command(_hash(name.c_str(), name.length()), (uint8_t *)command_data,
                    command_data_len, send_timeout)
          ? __get_result((uint8_t **)&result_pointer, &result_size,
                         recv_timeout)
          : false;
...
bool rpc_master::call(const char *name, void *command_data,
                      size_t command_data_len, void *result_data,
                      size_t result_data_len,
                      bool return_false_if_received_data_is_zero,
                      unsigned long send_timeout, unsigned long recv_timeout) {
...
bool rpc_slave::__get_command(uint32_t *command, uint8_t **data, size_t *size,
                              unsigned long timeout) {
  ...
  while ((millis() - start) < timeout) {
    _set_packet(__out_command_header_ack, _COMMAND_HEADER_PACKET_MAGIC, NULL,
                0);  // set repeatedly because SPI.transfer() clears.
    _zero(__in_command_header_buf, sizeof(__in_command_header_buf));
    _flush();
    if (_get_packet(_COMMAND_HEADER_PACKET_MAGIC, __in_command_header_buf,
      ...
      uint32_t in_command_data_buf_len =
          unpack_unsigned_long(__in_command_header_buf + 6) + 4;
      if (__buff_len < in_command_data_buf_len) return false;
      put_bytes(__out_command_header_ack, sizeof(__out_command_header_ack),
                _put_short_timeout);
      if (_get_packet(_COMMAND_DATA_PACKET_MAGIC, __buff,
                      in_command_data_buf_len, _get_long_timeout)) {
        put_bytes(__out_command_data_ack, sizeof(__out_command_data_ack),
                  _put_short_timeout);
        *command = cmd;
        *data = __buff + 2;
...
bool rpc_slave::__put_result(uint8_t *data, size_t size,
                             unsigned long timeout) {
  ...
  while ((millis() - start) < timeout) {
    _set_packet(out_header, _RESULT_HEADER_PACKET_MAGIC, (uint8_t *)header,
                4);  // set repeatedly because SPI.transfer() clears.
    _zero(__in_response_header_buf, sizeof(__in_response_header_buf));
    _zero(__in_response_data_buf, sizeof(__in_response_data_buf));
    _flush();
    if (_get_packet(_RESULT_HEADER_PACKET_MAGIC, __in_response_header_buf,
                    sizeof(__in_response_header_buf), _get_short_timeout)) {
      put_bytes(out_header, sizeof(out_header), _put_short_timeout);
      if (_get_packet(_RESULT_DATA_PACKET_MAGIC, __in_response_data_buf,
                      sizeof(__in_response_data_buf), _get_short_timeout)) {
        put_bytes(__buff, size + 4, _put_long_timeout);
        return true;
      }
    }
...
bool rpc_slave::__register_callback(uint32_t hash, rpc_callback_type_t type,
                                    void *value) {
...
bool rpc_slave::register_callback(const __FlashStringHelper *name,
...
bool rpc_slave::register_callback(const String &name, rpc_callback_t callback) {
  return __register_callback(_hash(name.c_str(), name.length()), __CALLBACK,
                             (void *)callback);
}
...
bool rpc_slave::register_callback(const __FlashStringHelper *name,
                                  rpc_callback_with_args_t callback) {
  return __register_callback(_hash(name), __CALLBACK_WITH_ARGS,
                             (void *)callback);
}

bool rpc_slave::register_callback(const String &name,
                                  rpc_callback_with_args_t callback) {
  return __register_callback(_hash(name.c_str(), name.length()),
                             __CALLBACK_WITH_ARGS, (void *)callback);
}

bool rpc_slave::register_callback(const char *name,
                                  rpc_callback_with_args_t callback) {
  return __register_callback(_hash(name), __CALLBACK_WITH_ARGS,
                             (void *)callback);
}
...
bool rpc_slave::register_callback(const __FlashStringHelper *name,
                                  rpc_callback_returns_result_t callback) {
  return __register_callback(_hash(name), __CALLBACK_RETURNS_RESULT,
                             (void *)callback);
}
...
bool rpc_slave::register_callback(const String &name,
                                  rpc_callback_returns_result_t callback) {
  return __register_callback(_hash(name.c_str(), name.length()),
                             __CALLBACK_RETURNS_RESULT, (void *)callback);
}
...
bool rpc_slave::register_callback(
    const __FlashStringHelper *name,
    rpc_callback_returns_result_no_copy_t callback) {
  return __register_callback(_hash(name), __CALLBACK_RETURNS_RESULT_NO_COPY,
                             (void *)callback);
}

bool rpc_slave::register_callback(
    const String &name, rpc_callback_returns_result_no_copy_t callback) {
  return __register_callback(_hash(name.c_str(), name.length()),
...
bool rpc_slave::register_callback(
    const char *name, rpc_callback_returns_result_no_copy_t callback) {
  return __register_callback(_hash(name), __CALLBACK_RETURNS_RESULT_NO_COPY,
                             (void *)callback);
}
...
bool rpc_slave::register_callback(
    const __FlashStringHelper *name,
    rpc_callback_with_args_returns_result_t callback) {
  return __register_callback(_hash(name), __CALLBACK_WITH_ARGS_RETURNS_RESULT,
                             (void *)callback);
}

bool rpc_slave::register_callback(
    const String &name, rpc_callback_with_args_returns_result_t callback) {
  return __register_callback(_hash(name.c_str(), name.length()),
...
bool rpc_slave::register_callback(
    const char *name, rpc_callback_with_args_returns_result_t callback) {
  return __register_callback(_hash(name), __CALLBACK_WITH_ARGS_RETURNS_RESULT,
                             (void *)callback);
}
...
bool rpc_slave::register_callback(
    const __FlashStringHelper *name,
    rpc_callback_with_args_returns_result_no_copy_t callback) {
  return __register_callback(_hash(name),
...
bool rpc_slave::loop(unsigned long recv_timeout, unsigned long send_timeout) {
  ...
  if (__get_command(&command, &data, &size, recv_timeout)) {
    ...
    for (size_t i = 0; i < __dict_alloced; i++) {
      if ((__dict[i].object == this) && (__dict[i].key == command) &&
          __dict[i].value) {
        switch (__dict[i].type) {
          ...
          case __CALLBACK_RETURNS_RESULT: {
            (rpc_callback_returns_result_t(__dict[i].value))((void **)&out_data,
                                                             &out_data_len);
            break;
          }
          case __CALLBACK_RETURNS_RESULT_NO_COPY: {
            out_data_len = (rpc_callback_returns_result_no_copy_t(
                __dict[i].value))(__buff + 2);
            break;
          }
          case __CALLBACK_WITH_ARGS_RETURNS_RESULT: {
            (rpc_callback_with_args_returns_result_t(__dict[i].value))(
                data, size, (void **)&out_data, &out_data_len);
            break;
          }
          case __CALLBACK_WITH_ARGS_RETURNS_RESULT_NO_COPY: {
            out_data_len = (rpc_callback_with_args_returns_result_no_copy_t(
                __dict[i].value))(data, size, __buff + 2);
            break;
          }
        }
...
bool rpc_can_master::get_bytes(uint8_t *buff, size_t size,
                               unsigned long timeout) {
  size_t i = 0;
  unsigned long start = millis();
...
bool rpc_can_master::put_bytes(uint8_t *data, size_t size,
                               unsigned long timeout) {
  size_t i = 0;
  unsigned long start = millis();
...
bool rpc_can_slave::get_bytes(uint8_t *buff, size_t size,
                              unsigned long timeout) {
  size_t i = 0;
  unsigned long start = millis();
...
bool rpc_can_slave::put_bytes(uint8_t *data, size_t size,
                              unsigned long timeout) {
  size_t i = 0;
  unsigned long start = millis();
...
bool rpc_i2c_master::get_bytes(uint8_t *buff, size_t size,
                               unsigned long timeout) {
  ...
  for (size_t i = 0; i < size; i += 32) {
    ...
    if (Wire.requestFrom(__slave_addr, request_size, request_stop) !=
        request_size) {
      ok = false;
      break;
    }
...
bool rpc_i2c_master::put_bytes(uint8_t *data, size_t size,
                               unsigned long timeout) {
  // Turn the bus on and off so as to prevent lockups.
  (void)timeout;
  bool ok = true;
  ...
  for (size_t i = 0; i < size; i += 32) {
    ...
    if ((Wire.write(data + i, request_size) != request_size) ||
        Wire.endTransmission(request_stop)) {
      ok = false;
      break;
    }
  }
...
void rpc_i2c_slave::onReceiveHandler(int numBytes) {
  ...
  for (int i = 0, j = min(__bytes_size, (uint32_t)numBytes); i < j; i++)
    __bytes_buff[i] = Wire.read();
...
void rpc_i2c_slave::onRequestHandler() {
  if (!__bytes_size) return;
  size_t written =
      Wire.write((const uint8_t *)__bytes_buff, min(__bytes_size, 32));
...
bool rpc_i2c_slave::get_bytes(uint8_t *buff, size_t size,
                              unsigned long timeout) {
  __bytes_buff = buff;
  __bytes_size = size;
  unsigned long start = millis();
...
bool rpc_i2c_slave::put_bytes(uint8_t *data, size_t size,
                              unsigned long timeout) {
  __bytes_buff = data;
  __bytes_size = size;
  unsigned long start = millis();
...
bool rpc_spi_master::get_bytes(uint8_t *buff, size_t size,
                               unsigned long timeout) {
...
bool rpc_spi_master::put_bytes(uint8_t *data, size_t size,
                               unsigned long timeout) {
  (void)timeout;
...
#define RPC_HARDWARE_SERIAL_UART_MASTER_IMPLEMENTATION(name, port)            \
  void rpc_hardware_serial##name##_uart_master::_flush() {                    \
    for (int i = port.available(); i > 0; i--) port.read();                   \
                                                                              ...
                                                                              \
  bool rpc_hardware_serial##name##_uart_master::get_bytes(                    \
      uint8_t *buff, size_t size, unsigned long timeout) {                    \
    port.setTimeout(timeout + 1);                                             \
    bool ok =                                                                 \
        (port.readBytes((char *)buff, size) == size) && (!_same(buff, size)); \
    if (!ok) delay(_get_short_timeout);                                       \
    return ok;                                                                \
  }                                                                           \
                                                                              \
  bool rpc_hardware_serial##name##_uart_master::put_bytes(                    \
      uint8_t *buff, size_t size, unsigned long timeout) {                    \
    (void)timeout;                                                            \
...
#define RPC_HARDWARE_SERIAL_UART_SLAVE_IMPLEMENTATION(name, port) \
  void rpc_hardware_serial##name##_uart_slave::_flush() {         \
    for (int i = port.available(); i > 0; i--) port.read();       \
                                                                  ...
                                                                  \
  bool rpc_hardware_serial##name##_uart_slave::get_bytes(         \
      uint8_t *buff, size_t size, unsigned long timeout) {        \
    port.setTimeout(timeout + 1);                                 \
    return port.readBytes((char *)buff, size) == size;            \
  }                                                               \
                                                                  \
  bool rpc_hardware_serial##name##_uart_slave::put_bytes(         \
      uint8_t *buff, size_t size, unsigned long timeout) {        \
    (void)timeout;                                                \
    return port.write((char *)buff, size) == size;                \
  }
...
bool rpc_software_serial_uart_master::get_bytes(uint8_t *buff, size_t size,
                                                unsigned long timeout) {
  (void)timeout;
  __serial.listen();
...
bool rpc_software_serial_uart_master::put_bytes(uint8_t *buff, size_t size,
                                                unsigned long timeout) {
  (void)timeout;
...
bool rpc_software_serial_uart_slave::get_bytes(uint8_t *buff, size_t size,
                                               unsigned long timeout) {
  (void)timeout;
  __serial.listen();
...
bool rpc_software_serial_uart_slave::put_bytes(uint8_t *buff, size_t size,
                                               unsigned long timeout) {
  (void)timeout;
  return __serial.write(buff, size) == size;
}
